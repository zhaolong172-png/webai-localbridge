...
9.8 Logs and Private Data

WebAI LocalBridge generates runtime logs. Logs may contain:
...
• File paths or partial file names.

The log directory is usually:

.mcp-logs/

Logs should not be packaged into releases, nor should they be committed to a Git repository.

If error logs need to be shared with others, it is recommended to first check whether they contain:
...
• Project names.
• File names.
• Sensitive content in command output.
• Business data.
• Account information.

For logs containing private or business data, sensitive information should be redacted before sharing.

Recommended release exclusions:
...
WebAI LocalBridge can be used for remote file browsing, remote assistance, and AI local operations, provided that the user clearly understands which directory is currently exposed, which service is enabled, and which types of operations are allowed.
